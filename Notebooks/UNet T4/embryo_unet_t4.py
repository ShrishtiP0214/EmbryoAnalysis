...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        logits = self.conv10(c9)
        return logits 

# Custom Dataset with Augmentation for t4 phase
class EmbryoT4Dataset(Dataset):
    def __init__(self, base_paths, target_path, phase_csv_dir, embryo_ids, transform=None, num_t4_embryos=50, num_other_embryos=0):
        if len(base_paths) != 6:
        ...
        print(f"Found {len(t4_embryos)} embryos with t4 phase")
        ...
        for eid in [eid for eid, _, _ in self.t4_embryos] + self.other_embryos:
            ...
            self.embryo_to_frames[eid] = list(range(1, len(image_files) + 1))
            self.embryo_to_frame_files[eid] = {i+1: image_files[i] for i in range(len(image_files))}
        
        self.samples = []
        for eid, start, end in self.t4_embryos:
            available_t4_frames = [f for f in self.embryo_to_frames[eid] if start <= f <= end]
            print(f"Embryo {eid}: t4 frames available: {len(available_t4_frames)}")
            selected_frames = available_t4_frames  # Use all t4 frames
    ...
    def __getitem__(self, idx):
        ...
        focal_stack = np.stack(focal_images, axis=-1)  # shape (H, W, 6)
        
        target_img_path = os.path.join(self.target_path, embryo_id, filename)
        target_image = Image.open(target_img_path).convert('L')
        target_image = np.array(target_image)  # shape (H, W)
        
        augmented = self.transform(image=focal_stack, target=target_image)
        input_tensor = augmented['image'].float()  # shape (6, H, W)
        target_tensor = augmented['target'].float()  # shape (1, H, W)
        
        # Normalize to [0, 1]
        input_tensor = input_tensor.float()  # Shape: (channels, height, width)
        target_tensor = target_tensor.float()  # Shape: (1, height, width)
...
def gradient_loss(output, inputs):
    # Sobel kernels on whatever device we're using
    ...
    # Compute gradient magnitude of the output
    gx_out = F.conv2d(output, sobel_x, padding=1)
    gy_out = F.conv2d(output, sobel_y, padding=1)
    grad_output = torch.sqrt(gx_out**2 + gy_out**2 + 1e-6)
    grad_output = torch.clamp(grad_output, 0.0, 100.0)
    grad_output = torch.nan_to_num(grad_output, nan=0.0, posinf=100.0, neginf=0.0)

    # Now compute per-channel gradients on the inputs and keep the maximum
    channel_grads = []
    for c in range(inputs.shape[1]):
        inp_c = inputs[:, c:c+1, :, :]
        gx_in = F.conv2d(inp_c, sobel_x, padding=1)
        gy_in = F.conv2d(inp_c, sobel_y, padding=1)
        g = torch.sqrt(gx_in**2 + gy_in**2 + 1e-6)
        g = torch.clamp(g, 0.0, 100.0)
        g = torch.nan_to_num(g, nan=0.0, posinf=100.0, neginf=0.0)
        channel_grads.append(g)

    # Stack and take max over the channel dimension
    max_grad_in = torch.stack(channel_grads, dim=1).amax(dim=1)

    # Finally the loss is the mean amount by which the input’s edges exceed the output’s
    diff = torch.clamp(max_grad_in - grad_output, min=0.0)
    diff = diff / 8.0 
    return diff.mean()


def combined_loss(output, target, inputs, lambda_grad=0.01):
    ...
    print(f"  → dice: {dice:.4f}, l1: {l1:.4f}, grad: {grad_loss:.4f}")
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            if torch.isnan(inputs).any() or torch.isinf(inputs).any():
                print("Found NaN or Inf in inputs!")
            if torch.isnan(targets).any() or torch.isinf(targets).any():
                print("Found NaN or Inf in targets!")
            break
        for inputs, targets in train_loader:
...
# Testing Function
def test_single_embryo(model, image_paths, transform, device):
    model.eval()
    focal_tensors = []
    for path in image_paths:
        img = Image.open(path).convert('L')
        img_tensor = transform(image=np.array(img))['image'].float()  # Ensure float32
...
def main_train(seed=42):
    ...
    target_path = r"C:\Projects\Embryo\Dataset\embryo_dataset"  # Ground truth as F0
    ...
    dataset = EmbryoT4Dataset(
        ...
        transform=transform,
        # num_t4_embryos=50,  # Adjust as needed
        num_other_embryos=0
    )
